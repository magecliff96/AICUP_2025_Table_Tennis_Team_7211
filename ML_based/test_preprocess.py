...
info_df = pd.read_csv(f'../dataset/39_Test_Dataset/{Type}_info.csv')
...
for _, row in info_df.iterrows():
    unique_id = row['unique_id']
    mode = row['mode']
    data_path = f'../dataset/39_Test_Dataset/{Type}_data/{unique_id}.txt'
...
testing_df = info_df[['unique_id', 'mode']].copy()
testing_df['period'] = period
testing_df.to_csv(f"{Type}_data.csv", index = None)
...
training_df.to_csv("test_data_v2.csv", index=False)
